...
yesButton.addEventListener('click', function(){
    window.location.href = `/match/contest/join?matchId=${matchId}&contestId=${contestId}`;
})
...
saveButton.addEventListener('click',function(){
    ...
    if(addedPlayers.length==11){
        ...
        window.location.href = `/match/contest/team?id=${matchId}&teamArray=${players}&captainId=${captainId}&viceCaptainId=${viceCaptainId}`;
    }else{
...
for(let i = 0; i < contestCard.length; i++){
    ...
    contestCard[i].addEventListener('click', function(){
        if(isLeaderboardVisible){
            console.log(contestId); 
            window.location.href = `/match/contest/leaderboard?matchId=${matchId}&contestId=${contestId}`;
        }else{
...
createContestSaveButton.addEventListener('click', function(){
    ...
    window.location.href = `/match/contest/create-contest?entryAmount=${entryAmount}&spots=${spots}&winners=${winners}&matchId=${matchId}`;
})
...
joinContestSaveButton.addEventListener('click',function(){
    ...
    window.location.href = `/match/contest/join-contest?joinCode=${joinCode}&matchId=${matchId}`;
})
